...
SELECTOR_MIN_QUOTE_VOLUME_USDC_24H = _selector_setting("min_quote_volume_usdc_24h", "SELECTOR_MIN_QUOTE_VOLUME_USDC_24H", 1000000, float)
SELECTOR_MIN_TRADE_COUNT_24H = _selector_setting("min_trade_count_24h", "SELECTOR_MIN_TRADE_COUNT_24H", 5000, int)
...
def get_market_stats_map():
    ...
    for sym, meta in _PAIR_META.items():
        try:
            ...
            session_change = ((last - session_open) / session_open * 100.0) if session_open > 0 else None
            out[sym] = {
                "last_price": last,
                "quote_volume_24h": volume * vwap,
                "trade_count_24h": int((row.get("t") or [0, 0])[-1] or 0),
                # Kraken ticker ``o`` is the current UTC session open, not a
                # rolling 24-hour price.  It remains observability only.
    ...
    *,
    ...
    for sym in symbols:
        ...
        if int(market["trade_count_24h"]) < SELECTOR_MIN_TRADE_COUNT_24H:
            ...
            continue
        change_24h_raw = market.get("change_pct_24h")
        if change_24h_raw is None:
            reject(sym, "change_24h_unavailable")
            continue
...
def _log_tradable_universe(symbols, eligible_symbols, filter_counts) -> None:
    print(
        ...
        f"reject_change24h_unavailable={filter_counts['change_24h_unavailable']} "
        f"reject_change24h={filter_counts['change_24h']} "
...
def _recent_ohlc_bars(symbol: str) -> list:
    return _recent_ohlc_bars_for_interval(symbol, interval=1)


def _recent_ohlc_bars_for_interval(symbol: str, interval: int) -> list:
...
def rolling_24h_change_pct(symbol: str) -> float | None:
    """Calculate a rolling 24-hour return from Kraken 5-minute OHLC bars."""
    try:
        # 288 completed five-minute bars cover a 24-hour observation window.
        bars = _recent_ohlc_bars_for_interval(symbol, interval=5)
        if not isinstance(bars, list) or len(bars) < 288:
            return None
        window = bars[-288:]
        opening = float(window[0][1])
        closing = float(window[-1][4])
        if opening <= 0:
            return None
...
def _attach_rolling_24h_changes(symbols: list[str], market_map: dict) -> None:
    """Populate rolling 24-hour returns only after basic liquidity gates pass."""
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as pool:
        futures = {pool.submit(rolling_24h_change_pct, sym): sym for sym in symbols}
        for future in as_completed(futures):
            ...
            if symbol in market_map:
                market_map[symbol]["change_pct_24h"] = change
...
def collect_candidates(excluded_symbols: set[str] | None = None, positive_only: bool = True):
    ...
    # Apply all filters that do not require candle history first.  This keeps
    # Kraken OHLC traffic bounded to a small liquid universe.
    prelim_market_map = {symbol: {**market, "change_pct_24h": 0.0} for symbol, market in market_map.items()}
    prelim_symbols, _ = _tradable_symbols(
        symbols,
        spread_map,
        prelim_market_map,
        excluded_symbols,
    )
    _attach_rolling_24h_changes(prelim_symbols, market_map)
