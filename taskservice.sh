...
show_bot_status() {
  echo "== systemd =="
  sudo systemctl status kraken-aifout-bot.service --no-pager
...
PY
  ...
  echo
  echo "== recent errors =="
  journalctl -u kraken-aifout-bot.service --since "15 minutes ago" --no-pager \
    | grep -Ei "traceback|exception|error|failed|critical" || true
}

case "${1:-}" in
  1)
    journalctl -u kraken-token-profile-selector.service -n 20 --no-pager
    ;;
  2)
    show_bot_status
    ;;
  3)
    sudo systemctl start kraken-aifout-bot.service
    ;;
  4)
    sudo systemctl stop kraken-aifout-bot.service
    ;;
  5)
    sudo systemctl status kraken-botdash.service --no-pager
    ;;
  6)
    sudo systemctl stop kraken-botdash.service
    ;;
  7)
    sudo systemctl start kraken-botdash.service
    ;;
  *)
