  ...
  return {
    ...
    async stopSelector() {
      ...
      try {
        ...
        this.control = d.control || this.control;
        this._log('kraken-token-profile-selector.timer', 'stop-auto', true, 'manual mode');
        await this.fetchServices();
      } catch (e) {
        this._log('kraken-token-profile-selector.timer', 'stop-auto', false, e.message);
      } finally {
        this.controlBusy = false;
      }
    },
    ...
    async startSelector() {
      ...
      try {
        ...
        if (!r.ok || !d.ok) throw new Error(d.error || r.statusText);
        this.control = d.control || this.control;
        this._log('kraken-token-profile-selector.timer', 'start-auto', true, 'auto mode');
        await this.fetchServices();
      } catch (e) {
        this._log('kraken-token-profile-selector.timer', 'start-auto', false, e.message);
      } finally {
        this.controlBusy = false;
      }
    },
    ...
    async restartBot() {
      ...
      try {
        ...
        if (!r.ok || !d.ok) throw new Error(d.error || d.output || r.statusText);
        this.control = d.control || this.control;
        this._log('kraken-aifout-bot.service', 'restart', true, d.output || '');
        await this.fetchServices();
      } catch (e) {
        this._log('kraken-aifout-bot.service', 'restart', false, e.message);
      } finally {
        this.controlBusy = false;
      }
    },
