...
"""
Kraken AiFout Bot Dashboard — Flask application
...
load_dotenv("/opt/kraken-aifout-bot/dashboard/botdash.env")

BASE_DIR = Path(os.environ.get("BOT_BASE_DIR", "/opt/kraken-aifout-bot"))
...
def get_control_state() -> dict:
    selector_timer = read_unit_state("kraken-token-profile-selector.timer")
    selector_service = read_unit_state("kraken-token-profile-selector.service")
    bot_service = read_unit_state("kraken-aifout-bot.service")
...
def get_fx_usdc_eur() -> Optional[float]:
    try:
        ...
        usdc_per_eur = float((row.get("c") or [0])[0])
        if usdc_per_eur <= 0:
            return None
...
def get_kraken_mid_price(symbol: str) -> tuple[Optional[float], Optional[float], Optional[float]]:
    try:
        pair = (symbol or "").upper().replace("/", "").replace("-", "")
...
def api_control():
    ...
    unit = data.get("unit", "kraken-aifout-bot.service")
            ...
            try:
                ...
                if now - last_push >= 2:
                    ...
                    if pos:
                        ...
                        current_price = None
                        try:
                            current_price, bid, ask = get_kraken_mid_price(symbol)
                        except Exception:
                            pass
...
def api_bot_manual_token():
    ...
    restart_result = None
    if restart:
        ok, output = systemctl("restart", "kraken-aifout-bot.service")
        restart_result = {"unit": "kraken-aifout-bot.service", "action": "restart", "ok": ok, "output": output}
...
def api_bot_selector_action(action: str):
    ...
    if action == "stop":
        ...
        if action == "restart":
            ok, output = systemctl("restart", "kraken-token-profile-selector.service")
            results.append({"unit": "kraken-token-profile-selector.service", "action": "restart", "ok": ok, "output": output})
...
@require_token_auth
def api_bot_pause():
    ok, output = systemctl("stop", "kraken-aifout-bot.service")
