#!/usr/bin/env python3
"""One-shot public Kraken scanner for Token Radar."""
...
from exchange.symbols import _asset_display
...
BASE_URL = os.getenv("TOKEN_RADAR_BASE_URL", "https://api.kraken.com").rstrip("/")
QUOTE = os.getenv("TOKEN_RADAR_QUOTE", os.getenv("QUOTE_ASSET", "USDC")).upper()
...
EXCLUDED_BASE_ASSETS = {
    ...
    for item in os.getenv(
        ...
        "USDC,USDT,FDUSD,TUSD,USDP,DAI,USD1,USDE,BFUSD,USD0,USDD,PYUSD,RLUSD,EUR,USD",
    ).split(",")
    if item.strip()
}
...
def _load_symbols(session: requests.Session) -> set[str]:
    global _PAIR_META
    data = _public_get(session, "/0/public/AssetPairs")
    symbols = set()
    _PAIR_META = {}
    for pair_id, row in (data or {}).items():
        if not isinstance(row, dict):
            continue
        base_asset = _asset_display(row.get("base", ""))
        quote_asset = _asset_display(row.get("quote", ""))
        if quote_asset != QUOTE or _is_excluded_base(base_asset):
            continue
        status = str(row.get("status") or "online").lower()
        if status not in ("online", ""):
            continue
        symbol = f"{base_asset}{quote_asset}"
        ws_symbol = row.get("wsname") or f"{base_asset}/{quote_asset}"
        symbols.add(symbol)
        _PAIR_META[symbol] = {"pair_id": pair_id, "ws_symbol": ws_symbol, "base_asset": base_asset, **row}
    if not symbols:
        print(f"TOKEN_RADAR: NO_PAIRS_FOR_QUOTE quote={QUOTE}", file=sys.stderr)
    return symbols


def _market_maps(session: requests.Session) -> tuple[dict, dict]:
    tickers = _public_get(session, "/0/public/Ticker")
    ticker_map = {}
    book_map = {}
    for symbol, meta in _PAIR_META.items():
        row = (tickers or {}).get(meta["pair_id"])
        if not isinstance(row, dict):
            continue
        bid = ((row.get("b") or [0]) or [0])[0]
        ask = ((row.get("a") or [0]) or [0])[0]
        last = ((row.get("c") or [0]) or [0])[0]
        volume = ((row.get("v") or [0, 0]) or [0, 0])[-1]
        vwap = ((row.get("p") or [0, 0]) or [0, 0])[-1]
        high = ((row.get("h") or [0, 0]) or [0, 0])[-1]
        low = ((row.get("l") or [0, 0]) or [0, 0])[-1]
        normalized = {
            "symbol": symbol,
            "lastPrice": last,
            "volume": volume,
            "quoteVolume": _float(volume) * (_float(vwap) or _float(last)),
            "priceChangePercent": 0.0,
            "highPrice": high,
            "lowPrice": low,
        }
        ticker_map[symbol] = normalized
        book_map[symbol] = {"symbol": symbol, "bidPrice": bid, "askPrice": ask}
...
def _fetch_klines(session: requests.Session, symbol: str, interval: str, limit: int) -> list:
    interval_map = {"1m": 1, "1h": 60}
    pair = (_PAIR_META.get(symbol) or {}).get("pair_id", symbol)
    data = _public_get(
        session,
        "/0/public/OHLC",
        {"pair": pair, "interval": interval_map.get(interval, interval)},
    )
    rows = (data or {}).get(pair) or next((v for k, v in (data or {}).items() if k != "last"), [])
    out = []
...
def _build_base_candidate(symbol: str, ticker: dict, book: dict) -> dict | None:
    ...
    return {
        "symbol": symbol,
        "exchange": "kraken",
        "pair_id": (_PAIR_META.get(symbol) or {}).get("pair_id", ""),
        "ws_symbol": (_PAIR_META.get(symbol) or {}).get("ws_symbol", ""),
        "quote_asset": QUOTE,
        "price": price,
        "bid": bid,
        "ask": ask,
