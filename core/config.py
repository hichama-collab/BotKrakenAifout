...
def loadConfig() -> Config:
    ...
    if not dry and (not apiKey or not apiSecret):
        raise RuntimeError("KRAKEN_API_KEY/KRAKEN_API_SECRET required when DRY_RUN=0")
...
def validateRuntimeSafety(cfg: Config) -> Config:
    ...
    if str(getattr(cfg, "krakenEnv", "spot") or "").strip().lower() != "spot":
        raise RuntimeError("KRAKEN_ENV must be spot")
    if bool(getattr(cfg, "dryRun", False)):
        return cfg

    if not str(getattr(cfg, "apiKey", "") or "").strip() or not str(getattr(cfg, "apiSecret", "") or "").strip():
        raise RuntimeError("KRAKEN_API_KEY/KRAKEN_API_SECRET required for live runtime")
